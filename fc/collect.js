...
// 自包含版本：登录态从镜像内置文件读取（STATE_PATH），prev 比价快照驻留进程内存。
const { chromium } = require('playwright');
const fs = require('fs');
const path = require('path');

// 上一轮比价快照持久化到文件：适配 CloudRun 缩容0 / 云函数（冷启动可恢复，避免每次都当基线）
const STATE_DIR = process.env.STATE_DIR || '/app/state';
const PREV_SNAP_FILE = path.join(STATE_DIR, 'prev-snap.json');
function loadPrevSnap() { try { return JSON.parse(fs.readFileSync(PREV_SNAP_FILE, 'utf8')); } catch (e) { return null; } }
...
// 跨 tick 的上一帧快照（从文件加载，冷启动可恢复；首次为 null 建基线）
let prevSnap = loadPrevSnap();
...
async function collect() {
  if (!fs.existsSync(STATE_PATH)) {
    return { ok: false, reason: 'NO_STATE', message: '容器内缺少 browser-state.json（请重新构建镜像并注入登录态）' };
  }
  ...
  if (captures.length === 0) {
    return { ok: false, reason: 'SESSION_EXPIRED', message: '未捕获到数据，登录态可能已失效，请本地重新登录并重新构建镜像' };
  }
